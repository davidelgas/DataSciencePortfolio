            ...
            if search_mode in ["Forum Only", "Forum + Web"]:
                ...
                for i, idx in enumerate(indices[0]):
                    if idx < len(st.session_state.df):
                        ...
                        retrieved_docs.append(f"FORUM THREAD {i+1}:
{content_text}
...
ANSWER:"""
            ...
            if search_mode == "Forum Only":
                ...
                if forum_answer and web_answer and not forum_answer.startswith("Error") and not web_answer.startswith("Error"):
                    combined_prompt = f"""Combine these two answers to the question about BMW E9 \"{query}\". Prioritize information from the forum if available, and supplement with web information. If there's conflicting information, state it. :
